...
puts "#{has_location}"
  ...
  def self.get_Interviewers(params)
        ...
        if(params[:search_with]=="OR")
          skill_level = {"query":""+keyword+" match (skill:Skill"+skill_name+")<-[HE:Has_experience]-(interviewer:Interviewer)"+search_with_skill_ignore_case+" with DISTINCT interviewer as interviewer_data,collect(skill.name) as skill_matched match (interviewer_data)-[:Has_Expertise]-(expertise:Expertise"+expertise+"),(language:Languages"+language+")<-[:Understands"+proficiency+"]-(interviewer_data)-[:Has_Location]->(location:Location"+location+"),(interviewer_data)-[:Has_Knowledge]-(domain:Domains"+domain+")   RETURN DISTINCT interviewer_data {interviewers:interviewer_data{.*,matched:size(skill_matched)}} as object,size(skill_matched),interviewer_data.total_yrs_of_exp ORDER BY size(skill_matched) desc,interviewer_data.total_yrs_of_exp desc" ,"params":{"skills_list":params[:keyword] ? params[:keyword].split(/, |,/) : ".*" ,"level":params[:level] ? level_value :'',"domain":params[:domain] ? params[:domain] :'',"proficiency":params[:proficiency] ? params[:proficiency] :'',"language":params[:language] ? params[:language] :'',"location":params[:location] ? params[:location] :'',"total_yrs_of_exp": params[:total_yrs_of_exp] ? exp :'',"expertise":params[:expertise] ? params[:expertise] :''}}
          puts "======== OR start============="
          puts "========= OR END================="
          response = Neo4jModule.post(skill_level)
          JSON.parse(response.body)["data"].collect{|object| object[0]["interviewers"]}
          ...
          using_and = {"query":""+keyword+ " match (skill:Skill"+skill_name+")<-[HE:Has_experience]-(interviewer:Interviewer)"+search_with_skill_ignore_case+" with DISTINCT interviewer as interviewer_data,collect(skill.name) as skill_matched "+match_skills+"match("+node+")-[:Has_Expertise]-(expertise:Expertise"+expertise+"),(language:Languages"+language+")<-[:Understands"+proficiency+"]-("+node+")-[:Has_Location]->(location:Location"+location+"),("+node+")-[:Has_Knowledge]-(domain:Domains"+domain+") RETURN DISTINCT "+node+" {interviewers:"+node+"{.*,matched:"+ranking+"}} as object,"+node+".total_yrs_of_exp ORDER BY "+node+".total_yrs_of_exp desc" ,"params":{"skills_list":params[:keyword] ? params[:keyword].split(/, |,/) : ".*","skills_length":params[:keyword] ? params[:keyword].split(/, |,/).length : ".*" ,"level":params[:level] ? level_value :'',"domain":params[:domain] ? params[:domain] :'',"proficiency":params[:proficiency] ? params[:proficiency] :'',"language":params[:language] ? params[:language] :'',"location":params[:location] ? params[:location] :'',"total_yrs_of_exp": params[:total_yrs_of_exp] ? exp :'',"expertise":params[:expertise] ? params[:expertise] :''}}
          puts "========== AND Start==========="
          puts "#{using_and}"
          puts "=========== AND END ==============="
  ...
  def self.import_data
    ...
    CSV.foreach("graph-interviewers.csv",:headers => true) do |row|
      # puts "#{row[0]} ================#{row[8]}"
      unless ["sunil.aghi@gmail.com","ssri4740@gmail.com","suddhasatwa_ghosh@yahoo.com","sinha.meenu@gmail.com","vurasivaprasad@gmail.com","geethareddy6482@gmail.com","michael.r.rose.mba@gmail.com","harinam007@yahoo.com","lpottipadu@gmail.com","soniaba333@gmail.com","athiruml@gmail.com","billnemanick@hotmail.com"].include? row[1]
